...
function BEMHTML(options) {
  ...
  this._keepBemjson = options.keepBemjson;
}
...
BEMHTML.prototype.runMany = function runMany(arr) {
  ...
  if (this.canFlush) {
    for (var i = 0; i < arr.length; i++) {
      if (keepbj)
        out.concat(context._flush(this._run(arr[i])));
      else
    ...
    for (var i = 0; i < arr.length; i++) {
      if (keepbj)
        out = out.concat(this._run(arr[i]));
      else
        out += this._run(arr[i]);
    }
  }
...
BEMHTML.prototype.render = function render(context,
  ...
  if (!isBEM && !cls) {
    return keepbj ?
      out :
      this.renderClose(out, context, tag, attrs, isBEM, ctx, content);
  }
  ...
  if (isBEM) {
    ...
    _cls += entity.jsClass;
    _cls += this.buildModsClasses(entity.block, entity.elem, mods);
    ...
    if (cls)
      _cls += ' ' + (typeof cls === 'string' ?
                    utils.attrEscape(cls).trim() : cls);
  } else {
    if (cls)
      _cls += cls.trim ? utils.attrEscape(cls).trim() : cls;
  }
  ...
  if (keepbj)
    (out.attrs = attrs || {}).class = _cls;
  else
    out += ' class="' + _cls + '"';

  if (isBEM && jsParams) {
    var _attrs = utils.jsAttrEscape(JSON.stringify(jsParams));
    if (keepbj)
      out.attrs['data-bem'] = _attrs;
    else
      out += ' data-bem=\'' + _attrs + '\'';
  }
